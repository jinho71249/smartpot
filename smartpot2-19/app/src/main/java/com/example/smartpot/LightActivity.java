...
public class LightActivity extends AppCompatActivity {
    ...
    private Integer lGetTime; //전등을 언제 킬건지
    private Integer lGetTimer; //얼마나 오래 킬건지
    ...
    private void initListener(){
        ...
        reservButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {//예약버튼
                lGetTime=Integer.parseInt(whatTime.getText().toString());//전등을 언제 킬건지
                lGetTimer=Integer.parseInt(howLong.getText().toString());//얼마나 오래 킬건지

                Intent intent = new Intent();
                intent.putExtra("lGetTime",lGetTime); //메인화면으로 값전달
                intent.putExtra("lGetTimer",lGetTimer);
